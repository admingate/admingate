                    ...
                    <div v-if="!isLoading && !verified">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <p>
                                ...
                                <a href="https://codecanyon.net/user/admingate/portfolio" target="_blank" rel="nofollow">new license here</a> to get a new purchase code.</small>
                            </p>
                        </div>
                    </div>
                    <div v-if="!isLoading && verified">
        ...
        methods: {
            verifyLicense() {
                axios.get(this.verifyUrl)
                    ...
                    .catch(res =>  {
                        Admingate.handleError(res.response.data);
                        this.isLoading = false;
                    });
            },
            ...
            activateLicense() {
                ...
                axios.post(this.activateLicenseUrl, {purchase_code: this.purchaseCode, buyer: this.buyer, license_rules_agreement: this.licenseRulesAgreement})
                    .then(res =>  {
                        if (res.data.error) {
                            Admingate.showError(res.data.message);
                        } else {
                            this.verified = true;
                            this.license = res.data.data;
                            Admingate.showSuccess(res.data.message);
                        }
