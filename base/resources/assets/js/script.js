class Admingate {
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Admingate.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Admingate.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Admingate.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Admingate.showError(item);
                        });
                    });
                }
            } else {
                Admingate.showError(data.statusText);
            }
        }
    }
    ...
    static initResources() {
        ...
        Admingate.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Admingate.callScroll($('.list-item-checkbox'));
        }
    ...
    static initMediaIntegrate() {
        ...
        if (jQuery().rvMedia) {
            ...
            Admingate.gallerySelectImageTemplate = `<div class="list-photo-hover-overlay">
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Admingate.gallerySelectImageTemplate;
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Admingate.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Admingate.onDragTool);
            $(window).on('mouseup', Admingate.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Admingate.onDragTool);
        $(window).off('mouseup', Admingate.onReleaseTool);
    }
